...
// ─────────────────────────────────────────────
// Helper: build the default "2 wrong" state
// Steps 1 and 15 are swapped; all others are correct.
// ─────────────────────────────────────────────
function buildDefaultSteps(): RecipeStep[] {
  const steps = [...RECIPE_STEPS]
  // Swap index 0 (step id=1) with index 14 (step id=15)
  ;[steps[0], steps[14]] = [steps[14], steps[0]]
  return steps
}
...
export function RecipeSequencer({ onComplete }: { onComplete?: () => void }) {
  ...
  // Always reset to: step 15 at position 1, step 1 at position 15, rest correct.
  const resetToDefault = useCallback(() => {
    setShuffledSteps(buildDefaultSteps())
    setCompleted(false)
  }, [])
    ...
    return (
      ...
      >
        <header className="recipe-smart-header">
          <div className="recipe-smart-row">

            {/* LEFT — logo */}
            <div className="recipe-logo-box">
              <span className="recipe-logo-icon"></span>
              <span className="recipe-logo-text">SMaRT Simulator</span>
            </div>
            ...
            {/* CENTER — HOME */}
            <div className="recipe-home-center">
              <button
                className="recipe-home-link"
                onClick={() => router.push("/")}
              >
                HOME
              </button>
